...
#include "AbilitySystem/Abilities/ChargingShot.h"

FString UChargingShot::GetDescription(int32 Level)
{
...
FString UChargingShot::GetNextLevelDescription(int32 Level)
{
	const int32 ScaledDamage = Damage.GetValueAtLevel(Level);
	const float ManaCost = FMath::Abs(GetManaCost(Level));
	const float Cooldown = GetCooldown(Level);
